    ...
    def show_results(self, analysis: dict, result_type: str = "text"):
        ...
        if result_type == "text" or result_type == "parse":
            ...
            if analysis.get("design_score"):
                ...
                score_text = QLabel(analysis["design_score"])
                score_text.setWordWrap(True)
                score_text.setStyleSheet("color: #94a3b8;")
            ...
            # Целевая аудитория
            if analysis.get("focus_group"):
                fg_frame = QFrame()
                fg_frame.setObjectName("resultBlock")
                fg_layout = QVBoxLayout(fg_frame)
                
                fg_title = QLabel("👥 Целевая аудитория")
                fg_title.setObjectName("sectionTitle")
                fg_text = QLabel(analysis["focus_group"])
                fg_text.setWordWrap(True)
                fg_text.setStyleSheet("color: #94a3b8;")
                
                fg_layout.addWidget(fg_title)
                fg_layout.addWidget(fg_text)
                self.results_layout.addWidget(fg_frame)
